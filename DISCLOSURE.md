...
benchmark runner, robustness and C0-C6 difficulty-control
...
selected-model run planners, provider-specific experiment adapters, paper table
builders, or generated paper outputs.
...
with a `candidate` string field. `scripts/model_target.py` provides a neutral
OpenAI-compatible adapter driven by `ABW_MODEL_API_KEY`,
`ABW_MODEL_BASE_URL`, and `ABW_MODEL_ID`.

`scripts/run_experiment.py` is the main single-model entrypoint: it validates a
packaged dataset slice, runs one `--target-command` adapter against it, and
writes JSON results and a manifest with the resulting metrics and composite
score. It does not know about any
specific provider or model-selection matrix — pass any adapter command, e.g.
`python scripts/model_target.py`, and any model name through that
...
uses `scripts/generate_perturbed_dataset.py` and `scripts/robustness_plan.py` to
create paired perturbed datasets and evaluate them through the same
model-agnostic benchmark runner. C0-C6 difficulty shape experiments use
`scripts/build_paired_difficulty_dataset.py`. These workflows intentionally live
